    ...
    let content = html`
      <page-body title="${this.title}" category="${this.category}" description="${this.description}">
        ...
        <action-card title="DAY 4: Kitty Items Market - Sell Market Item"
          description="Sell a kitty item to the market" action="kittyItemsMarketSellMarketItem" method = "post" fields="signer itemID price" >
        <account-widget field="signer" label="Signer">
        </account-widget>
        <text-widget field="itemID" label="Item ID" placeholder="0">
        </text-widget>
        <text-widget field="price" label="Amount" placeholder="10.0">
        </text-widget>
        ...
        <action-card title="DAY 4: Kitty Items Market - Buy Market Item"
          description="Buy a kitty item from market" action="kittyItemsMarketBuyMarketItem" method="post" fields="signer itemID marketCollectionAddress">
        <account-widget field="signer" label="Signer">
        </account-widget>
        <text-widget field="itemID" label="Item ID" placeholder="0">
        </text-widget>
        ...
        description="Read the sale Collection ID to return number-my" action="kittyItemsMarketReadSaleCollectionIDs" method="get" fields="marketCollectionAddress">
        <account-widget field="marketCollectionAddress" label="Address">
        </account-widget>
        </action-card>
        ...
        <!--Day 5-->
        <action-card title="Day5: Burn Token And get NFT"
          description="You Cloud burn kibble token and get a kittyeItem" action="kibbleBurnTokens" 
          method="post" fields="recipient amount typeID">
          <account-widget field="recipient" label="Recipient">
          </account-widget>
          <text-widget field="amount" label="Amount" placeholder="20.0">
          </text-widget>
          <text-widget field="typeID" label="Tye ID" placeholder="5">
          </text-widget>
        </action-card>
