...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            assert.equal(res1.result,1,"user market collection should have one item")

            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res2.result[0],1,"user market collection should have a item with item type id is 1")
        })
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            assert.equal(res1.result, 5.0 , "Incorrect balance in account 0")

            // 6) assert the result from step 4) equals 25.0
            assert.equal(res2.result,25.0,"The amount should be 25")
        })

        it(`Check burned token and get items`,async()=>{
            let testData1 = {
                amount: "1.0",
                recipient: config.accounts[0],
                typeID:"10"
            ...
            await DappLib.kibbleBurnTokens(testData1)

            let res1 = await DappLib.kibbleGetSupply({})

            let res2 = await DappLib.kibbleGetBalance(testData2)

            let res3 = await DappLib.kittyItemsReadCollectionIDs(testData2)

            assert.equal(res1.result,29.0,"Admin has incorrect balance")
            assert.equal(res2.result,4.0,"Incorrect total supply")
            console.log(res3.result)
            assert.equal(res3.result[1],2,"Incorrect Collection ID in Kitty Item")
        })
